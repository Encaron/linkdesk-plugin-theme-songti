# 宋体印刷体（theme-songti）——LinkDesk 插件仓

> **本文件是给在这个仓里干活的 AI 看的**（Claude Code / Codex / Cursor / …）。人看 `README.md`。
> 插件身份的唯一来源 = `plugin.json` 顶层的 `pluginId`（本仓：`theme-songti`）。当前版本 `1.0.2`。

## 1. 这是什么

宋体印刷体——字族轴：整个 UI 走宋体（SimSun），纸白墨黑的文档感。
